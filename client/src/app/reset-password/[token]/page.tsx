...
export default function ResetPasswordPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 200) {
        setIsSuccess(true);
        setMessage("Password has been successfully reset! Redirecting to dashboard...");
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] bg-background px-4 py-8 relative overflow-hidden">
      ...
      >
        ...
        <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center text-white">Create New Password</h2>
        ...
        {isSuccess && (
          <div className="mt-4 flex justify-center">
             <Link href="/" className="px-6 py-2 rounded-md bg-primary/20 text-primary hover:bg-primary/30 transition-all">
                Go to Dashboard
             </Link>
          </div>
        )}
